...
describe('Calculator', () => {
  ...
  describe('with expression:',
    () => {
      ...
      afterEach(() => {
        ...
        expect(expectations).not.to.be.null;
        var calculationResult = Calculator.calculate(expectations.formula);
        expect(calculationResult.isValid).to.be.true;
        expect(calculationResult.errorPosition).to.be.null;
        expect(calculationResult.errorMessage).to.be.null;
        expect(calculationResult.result).to.equal(expectations.expectedResult);
      });
  ...
  describe('with correct formulas:',
    () => {
      ...
      afterEach(() => {
        ...
        expect(expectations).not.to.be.null;
        var calculationResult = Calculator.calculate(expectations.formula);
        expect(calculationResult.isValid).to.eq(!!calculationResult.errorMessage);
        expect(calculationResult.errorPosition).to.be.null;
        expect(calculationResult.errorMessage).to.be.null;
        ...
        //         expect(relation).to.equal(1);
        //     } else {
        //         fail('error');
        //     }
        // } else {
        //     expect(calculationResult.result).to.equal(expectations.expectedResult, 1);
        // }
      });
  ...
  describe('with invalid formulas:',
    () => {
      ...
      afterEach(() => {
        // Common code used in all tests
        expect(expectations).not.to.be.a('null');
        var calculationResult = Calculator.calculate(expectations.formula);
        expect(calculationResult.result).to.be.NaN;
        expect(calculationResult.isValid).to.be.false;
        expect(calculationResult.errorPosition).to.equal(expectations.expectedErrorPosition);
      });
